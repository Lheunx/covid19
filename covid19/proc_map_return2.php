...
if($i>0){
  echo '<div style="max-width:50px" id="'.$key->gubunEn.'" value="'.$key->gubunEn.'" name="button" data-city="map_city'.$i.'">
    <button type="button" name="button">
      <span class="name">'.$key->gubun.'</span>
      <br><span class="num">'.$key->defCnt.'</span>
      <br><span class="before">(+'.$key->incDec.')</span>
    </button>
  </div>';

  echo '<div style="display:none;" id="'.$key->gubunEn.'_s" >
  ...
  if ($i==10 or $i==15 or $i==18 or $i==17){
    ...
    $('div#".$key->gubunEn."').offset({top:offt,left:offl+50});
    </script>";
  }
  elseif ($i==8 or $i==2 or $i==12 ) {
    ...
    offt = $('path.map_city".$i."').offset().top;
    offl = $('path.map_city".$i."').offset().left;
    $('div#".$key->gubunEn."').offset({top:offt,left:offl-30});
    </script>";
  }
  else {
    $string = "<script>
    ...
    offt = $('path.map_city".$i."').offset().top;
    offl = $('path.map_city".$i."').offset().left;
    $('div#".$key->gubunEn."').offset({top:offt,left:offl});
    </script>";
  }
